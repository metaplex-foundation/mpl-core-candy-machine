...
pub fn update_asset_v1<'info>(
    ...
    process_update_asset(&mut ctx, update_args, label)
}

pub fn process_update_asset(
    ...
    for condition in &conditions {
        if let Err(error) = condition.validate(ctx, &guard_set, &update_args) {
            return process_error(ctx, &guard_set, error);
        }
    }
    ...
    for condition in &conditions {
        condition.pre_actions(ctx, &guard_set, &update_args)?;
    }

    cpi_update_asset(ctx)?;

    for condition in &conditions {
        condition.post_actions(ctx, &guard_set, &update_args)?;
    }
...
fn cpi_update_asset(ctx: &EvaluationContext) -> Result<()> {
    let candy_guard = &ctx.accounts.candy_guard;

    // candy machine mint instruction accounts
    let update_asset_accounts = Box::new(mpl_core_candy_machine_core::cpi::accounts::UpdateAsset {
    ...
    let update_asset_infos = update_asset_accounts.to_account_infos();
    let mut update_asset_metas = update_asset_accounts.to_account_metas(None);

    update_asset_metas.iter_mut().for_each(|account_meta| {
        if account_meta.pubkey == ctx.accounts.asset.key() {
            account_meta.is_signer = ctx.accounts.asset.is_signer;
        }
    });

    let args = mpl_core_candy_machine_core::instruction::UpdateAsset {
        args: mpl_core_candy_machine_core::UpdateAssetArgs {
            plugins: ctx.plugins.to_vec(),
        },
    };
    let arg_data = args.try_to_vec()?;

    let data = mpl_core_candy_machine_core::instruction::UpdateAsset::DISCRIMINATOR
        .to_vec()
        .iter()
        .cloned()
    ...
    let update_asset_ix = Instruction {
        program_id: mpl_core_candy_machine_core::ID,
        accounts: update_asset_metas,
        data,
    };
    ...
    invoke_signed(&update_asset_ix, &update_asset_infos, &signer)?;
