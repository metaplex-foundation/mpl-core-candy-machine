    ...
    'c: 'info,
{
    ...
    let candy_guard_account = if candy_guard.as_ref().data_is_empty() {
        None
    } else {
        let account: Account<CandyGuard> = Account::try_from(candy_guard.account_info())?;
        Some(account)
    };
    ...
    let candy_machine_account = if candy_machine.as_ref().data_is_empty() {
        None
    } else {
        let account: Account<CandyMachine> = Account::try_from(candy_machine.account_info())?;
