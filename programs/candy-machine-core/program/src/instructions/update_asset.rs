...
use mpl_core::{
    ...
    fetch_plugin,
    instructions::{
        AddPluginV1Cpi, AddPluginV1CpiAccounts, AddPluginV1InstructionArgs, UpdateV2CpiBuilder,
    },
...
use crate::{
    ...
    utils::*,
    CandyError, CandyMachine, ConfigLine, UpdateAssetArgs,
};

/// Accounts to Update an Asset.
pub(crate) struct UpdateAccounts<'info> {
...
pub(crate) fn process_update_asset(
    candy_machine: &mut Box<Account<'_, CandyMachine>>,
    accounts: UpdateAccounts,
    bump: u8,
    mint_args: &UpdateAssetArgs,
) -> Result<()> {
    let asset = BaseAssetV1::deserialize(&mut &accounts.asset.data.borrow_mut()[..]);

    match asset {
        Ok(asset) => {
            if asset.owner != accounts.asset_owner.key() && !accounts.asset_owner.is_signer {
                return err!(CandyError::IncorrectOwner);
            }
            Ok(())
        }
        Err(_) => err!(CandyError::AssetMustBeInitialized),
    }?;
    ...
    let (auth, update_delegate, _) = fetch_plugin::<BaseCollectionV1, UpdateDelegate>(
        &accounts.collection,
        PluginType::UpdateDelegate,
    )?;

    assert_plugin_pubkey_authority(&auth, &update_delegate, &accounts.authority_pda.key())?;
...
/// Updates the Asset account to the new Asset.
fn update(
    candy_machine: &mut Box<Account<'_, CandyMachine>>,
    accounts: UpdateAccounts,
    bump: u8,
    ...
    UpdateV2CpiBuilder::new(&accounts.mpl_core_program)
    ...
    for plugin in plugins {
        AddPluginV1Cpi::new(
            ...
            AddPluginV1InstructionArgs {
                ...
                init_authority: plugin.authority.clone(),
            },
        )
        .invoke_signed(&[&authority_seeds])
        .map_err(anchor_lang::error::Error::from)?;
